...
if [[ ! -f "$NELL_INPUT" ]]; then
  ...
  echo "prepare the pilot split first; see README.md" >&2
  exit 1
fi
if [[ -e "$RUN_DIR" ]]; then
...
mkdir -p "$RUN_DIR"
cp "$VERSION_DIR/configs/nell23k_pilot_qwen05b.json" "$RUN_DIR/config.json"
...
echo "$RUN_DIR" > "$VERSION_DIR/results/LAST_NELL23K_PILOT_RUN.txt"
echo "NELL23K pilot completed: $RUN_DIR"
