...
if [[ -z "${RUN_DIR:-}" ]]; then
  if [[ -f "$VERSION_DIR/results/LAST_NELL23K_DIAGNOSTIC_CROSS_RUN.txt" ]]; then
    RUN_DIR="$(cat "$VERSION_DIR/results/LAST_NELL23K_DIAGNOSTIC_CROSS_RUN.txt")"
  elif [[ -f "$VERSION_DIR/results/LAST_NELL23K_PILOT_RUN.txt" ]]; then
