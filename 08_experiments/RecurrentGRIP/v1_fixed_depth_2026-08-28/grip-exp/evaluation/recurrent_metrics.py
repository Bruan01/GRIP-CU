...
def summarize_recurrent_predictions(rows: list[dict]) -> dict:
    ...
    buckets: dict[tuple[int, int, str], list[bool]] = defaultdict(list)
    for row in rows:
        key = (int(row["true_hop"]), int(row["recurrence_k"]), row.get("adapter_control", "correct"))
        buckets[key].append(bool(row["correct"]))
    ...
    by_hop_and_k = {
        ...
        for (hop, depth, control), values in sorted(buckets.items())
    }
    ...
    for row in correct_rows:
        ...
        hop_lookup[question_id] = int(row["true_hop"])
        if bool(row["correct"]):
    ...
    rho = spearman_correlation(
        [hop_lookup[question_id] for question_id in solved_ids],
        [best_depth_by_question[question_id] for question_id in solved_ids],
    )
    return {
        "count": len(rows),
        "accuracy": sum(bool(row["correct"]) for row in rows) / len(rows),
        "correct_adapter_accuracy": (
            sum(bool(row["correct"]) for row in correct_rows) / len(correct_rows) if correct_rows else 0.0
        ),
