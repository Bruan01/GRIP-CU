...
if [[ -z "${RUN_DIR:-}" ]]; then
  if [[ -f "$VERSION_DIR/results/LAST_PILOT_RUN.txt" ]]; then
    RUN_DIR="$(cat "$VERSION_DIR/results/LAST_PILOT_RUN.txt")"
  else
    echo "error: set RUN_DIR to a completed pilot run directory" >&2
    exit 1
  fi
fi
