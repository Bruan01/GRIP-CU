...
日期: 2026-09-20（补实验 I / I2 负采样空间审计）；2026-09-19（补实验 H smoke）；2026-09-16（补 7B 全量评测）；2026-09-10（0.5B smoke）；2026-09-09（H2 用 MLP storage adapter 重测）；2026-09-03（初稿）
...
负采样侧新证据（实验 I / I2）：官方 10-way 的干扰项是 198 词表上的均匀抽取。把 listed 负样本换成 Stage-1 余弦近邻（实验 I）后，全量 9895 题生成 EM 只 +0.82 pp，弱于均匀抽样的实验 H；离线审计显示余弦池命中真干扰项的质量（0.0042）甚至低于 pool 内均匀（0.0040），远低于全词表均匀（0.0457）。给关系嵌入做部分白化（实验 I2，`pca_whiten k=197 α=0.5`）能修好退化的几何（两两余弦 0.8776→−0.0049，有效负样本 39.1/40→15.5），但仍受「pool 只覆盖约 20% 干扰项」的上限约束，预期不如均匀抽样；训练 2026-09-20 开跑。
...
- ✅ 2026-09-19：实验 I 训完并做全量解码。listed 负样本改成「Stage-1 余弦 top-40 池内抽样」（`--listed_negative_source embed_sim`，pool=40、τ=0.1）。全量 9895：listed 生成 EM 85.84% vs 冻结 B1 85.02%（**+0.82 pp**），列表内错 1160→475、OOV 322→926。三种 listed 里最弱。目录 `results/runs/20260919_qwen7b_embed_negatives/`、`results/runs/20260919_qwen7b_full_listed_embed_negatives/`。
- ⏳ 2026-09-20：实验 I2。离线审计（`scripts/audit_relation_geometry.py`）确认官方干扰项在 198 词表上均匀分布，raw 余弦池的命中质量 0.0042 低于 pool 内均匀 0.0040；部分白化 `pca_whiten k=197 α=0.5` 把几何修好（余弦 0.8776→−0.0049、hub 10→5、有效负样本 39.1→15.5、命中质量→0.0072）。tmux `whiten-neg-20260920` 重训 listed（pool/τ/seed 不变），目录 `results/runs/20260920_qwen7b_whiten_negatives/`。
...
| 2026-09-19 实验 I 全量 | 7B | 全量 9895 | 85.84% vs 冻结 B1 85.02%（+0.82 pp） | 未做 | 余弦近邻负样本是最弱的一种；列表内 1160→475，OOV 322→926 |
| 2026-09-20 实验 I2 | 7B | 全量 9895 | 训练中（预期介于 +0.82 与均匀抽样之间） | 未做 | 部分白化修好几何但不改变干扰项分布；见 §4.9 |
...
实验 I 把负样本换成「Stage-1 觉得最像金关系」的关系后更差，原因可以离线量化，不需要猜：

- **嵌入空间退化。** 198×3584 的关系表两两余弦均值 +0.8776（标准差 0.047），top-1 近邻被 10 个关系轮流占据；τ=0.1、pool=40 时有效负样本 39.1/40，softmax 近乎均匀，「按相似度抽样」实际接近随机。
- **官方干扰项本来就是均匀抽的。** 只用训练切分（43 个金关系、565 条配对）：干扰项入 top-40 池率 0.162，低于随机 40 子集的理论值 0.203；相似度百分位 0.475（随机 0.500）；采样器命中真干扰项的质量 0.0042，低于 pool 内均匀 0.0040，远低于全词表均匀 0.0457。限制在相似度池里比均匀抽更不容易抽到官方干扰项，这正解释了 H（均匀）> I（余弦池）。
- **白化能修几何，但修不了分布。** 部分白化 `pca_whiten k=197 α=0.5` 把余弦 0.8776 拉到 −0.0049、hub 10→5、有效负样本 →15.5、命中质量 →0.0072，但 pool=40 的覆盖率上限（约 20%）与嵌入无关。全白化（k=197, α=1）是陷阱：行范数被拉平后所有非对角余弦相同，排序信息全丢（hub 32、有效负样本 40/40、近邻相关性跌到随机以下）。审计表见 `汇报_进展与实验结果.md` §4.10，原始数据 `results/relation_geometry_audit.json`。
...
6. **负采样要继续做，方向是「训练干扰项分布 = 评测干扰项分布」。** 官方 10-way 在 198 词表上均匀抽；实验 I/I2 证明加「更像金关系」的偏置反而降低命中真干扰项的质量。要进一步调，先动抽样分布，不要先动嵌入空间。
...
| `results/runs/20260919_qwen7b_embed_negatives/`、`results/runs/20260919_qwen7b_full_listed_embed_negatives/` | 实验 I：余弦近邻负样本训练产物 + 官方 9895 题解码（+0.82 pp） |
| `scripts/precompute_relation_embeddings.py` | 实验 I：用 Stage-1 adapter 编码 198 个训练图关系 |
| `scripts/whiten_relation_embeddings.py` | 实验 I2：线性变换（center / allbuttop / 部分 PCA 白化）+ 几何报告 |
| `scripts/audit_relation_geometry.py` | 实验 I2：各变换空间的离线审计（近邻相关性、官方干扰项命中质量） |
| `results/relation_geometry_audit.json` | 审计原始数据（gitignored，需重跑脚本生成） |
| `configs/run_listed_embed_negatives.sh`、`configs/run_listed_whiten_negatives.sh` | 实验 I / I2 的启动脚本（复用 20260913 Stage 1，冻结 B1） |
