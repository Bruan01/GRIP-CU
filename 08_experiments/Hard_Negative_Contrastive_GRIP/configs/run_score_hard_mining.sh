...
RUN_DIR="${RUN_DIR:-$HNG/results/runs/${RUN_ID}_score_hard_mining}"
OUTPUT="${OUTPUT:-$RUN_DIR/score_hard_manifest.jsonl}"
LIMIT="${LIMIT:-64}"
TMUX_SESSION="${TMUX_SESSION:-score-hard-${RUN_ID}}"
...
mkdir -p "$RUN_DIR"
{
  ...
  echo "output=$OUTPUT"
  echo "limit=$LIMIT"
  echo "hard_k=${HARD_K:-1}"
  echo "total_k=${TOTAL_K:-9}"
  echo "candidate_batch_size=${CANDIDATE_BATCH_SIZE:-8}"
  echo "seed=${SEED:-2026}"
...
echo "[launch] score-hard mining limit=$LIMIT tmux=${TMUX_SESSION:-none}" | tee -a "$RUN_DIR/run.log"
set +e
"$PYTHON" "$HNG/scripts/mine_score_hard_negatives.py" \
