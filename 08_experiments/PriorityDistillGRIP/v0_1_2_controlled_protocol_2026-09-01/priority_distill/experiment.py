...
from .supervision import build_method_supervision, build_oracle_evidence_prompt
...
def _prompt_for_condition(row: dict, condition: str) -> str:
    if condition == "graph_free":
        return build_evaluation_prompt(row["text"])
...
def generate_predictions(model, tokenizer, rows: list[dict], config: dict, device: torch.device, dtype: torch.dtype, condition: str, checkpoint_name: str) -> tuple[list[dict], dict]:
    model.eval()
    ...
    with torch.inference_mode():
        ...
        progress = tqdm(
            ...
            desc=f"{condition} eval {checkpoint_name}",
            unit="batch",
            leave=False,
            disable=bool(config.get("runtime", {}).get("disable_progress", False)),
        )
        for start in progress:
            ...
            prompts = [_prompt_for_condition(row, condition) for row in batch_rows]
            if condition == "graph_free" and any("Candidate evidence:" in prompt for prompt in prompts):
...
def evaluate_checkpoint(model, tokenizer, splits: dict[str, list[dict]], config: dict, device: torch.device, dtype: torch.dtype, output_dir: Path, checkpoint_name: str) -> dict:
    metrics = {}
    for condition in config["diagnostic"]["conditions"]:
        for split in config["diagnostic"]["splits"]:
            prediction_rows, split_metrics = generate_predictions(model, tokenizer, splits[split], config, device, dtype, condition, checkpoint_name)
