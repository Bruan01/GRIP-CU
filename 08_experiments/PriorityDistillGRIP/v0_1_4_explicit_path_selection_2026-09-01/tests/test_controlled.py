...
class ControlledConfigTests(unittest.TestCase):
    def test_direct_and_oracle_configs_are_distinct(self):
        ...
        anti_copy = load_config(ROOT / 'configs/candidate_selection_anti_copy.json')
        ...
        self.assertEqual(anti_copy['protocol'], 'candidate_selection_anti_copy')
        self.assertEqual(replay['training']['stage2_replay_ratio'], 0.2)
        self.assertEqual(anti_copy['training']['answer_only_learning_rate'], 0.0003)
