...
def _graph_free_trace_target(row: dict) -> tuple[str, int]:
    ...
    return f"Trace: {' -> '.join(trace_nodes)}\nAnswer: {row['answer']}", max(0, len(nodes) - 2)
...
def build_method_supervision(rows: list[dict], pools: dict[str, list[dict]], method: str, seed: int) -> list[dict]:
    ...
    for row in sorted(rows, key=lambda item: item["task_id"]):
        ...
        elif method == "graph_free_trace_terminal_masked":
            selected = []
            evidence_kind = "graph_free_structured_trace_target"
            prompt = _graph_free_trace_prompt(row["text"])
            target_text, intermediate_node_count = _graph_free_trace_target(row)
        ...
        supervision.append(
            {
                ...
                "trace_target": target_text if method == "graph_free_trace_terminal_masked" else None,
