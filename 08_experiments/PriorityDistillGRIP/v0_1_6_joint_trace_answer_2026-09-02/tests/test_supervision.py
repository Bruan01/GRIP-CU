...
class TraceSupervisionTests(unittest.TestCase):
    def setUp(self):
        self.rows = [make_row(0, ["r1"]), make_row(1, ["r1", "r2"]), make_row(2, ["r1", "r2", "r3"])]
        self.pools = {
            row["task_id"]: [{**row, "is_gold": True, "source_task_id": row["task_id"], "source_split": "train", "answer": row["answer"]}]
            for row in self.rows
        }

    def test_trace_keeps_intermediates_and_masks_terminal(self):
        records = build_method_supervision(self.rows, self.pools, "graph_free_trace_terminal_masked", seed=3)
        self.assertEqual(records[0]["target_text"], "Trace: n0_0 -> <MASKED_TERMINAL>\nAnswer: n0_1")
        self.assertEqual(records[1]["target_text"], "Trace: n1_0 -> n1_1 -> <MASKED_TERMINAL>\nAnswer: n1_2")
        self.assertEqual(records[2]["intermediate_node_count"], 2)
        self.assertTrue(all("Candidate" not in record["prompt"] for record in records))
        self.assertTrue(all(record["target_text"].splitlines()[0].endswith("<MASKED_TERMINAL>") for record in records))
...
class ExplicitSelectionSupervisionTests(unittest.TestCase):
    def setUp(self):
        self.rows = [make_row(i, ["r1", f"r{i}"]) for i in range(6)]
        self.pools = build_candidate_pools(self.rows, distractor_count=3, seed=3)

    def test_explicit_selection_has_numbered_target(self):
        record = build_method_supervision(self.rows, self.pools, "explicit_path_selection", seed=3)[0]
        self.assertIn("Selected path: <path number>", record["prompt"])
        self.assertRegex(record["target_text"], r"^Selected path: [1-4]\nAnswer: ")
        self.assertEqual(record["selection_target"], record["gold_position"] + 1)
        self.assertIn("Output:", record["prompt"])

    def test_explicit_anti_copy_hides_all_terminals(self):
        record = build_method_supervision(self.rows, self.pools, "explicit_path_selection_terminal_masked", seed=3)[0]
        self.assertIn("terminal entity hidden", record["prompt"])
        self.assertEqual(record["prompt"].count("<MASKED_TERMINAL>"), 4)
        self.assertEqual(record["prompt"].count("- "), 4)
        self.assertIn("Output:", record["prompt"])
