...
def parse_trace_output(text: str) -> str:
    """Extract the structured Trace line, returning an empty string if absent."""
    match = _TRACE_RE.search(str(text))
...
def score_predictions(rows: Iterable[Mapping]) -> dict:
    ...
    for row in rows:
        prediction = row.get("prediction_answer")
    ...
    result = _accuracy_summary(correct_values, by_depth)
    if any(row.get("expected_trace") is not None for row in rows):
        trace_values = [
            int(normalize_trace(str(row.get("prediction_trace", ""))) == normalize_trace(str(row["expected_trace"])))
            for row in rows
        ]
        trace_by_depth: dict[int, list[int]] = defaultdict(list)
        joint_values = []
        joint_by_depth: dict[int, list[int]] = defaultdict(list)
        for row, trace_ok, answer_ok in zip(rows, trace_values, correct_values):
            trace_by_depth[int(row["depth_label"])].append(trace_ok)
            joint = trace_ok * answer_ok
            joint_values.append(joint)
            joint_by_depth[int(row["depth_label"])].append(joint)
        result["trace"] = _accuracy_summary(trace_values, trace_by_depth)
        result["joint"] = _accuracy_summary(joint_values, joint_by_depth)
    return result
