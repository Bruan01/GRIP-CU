...
from .metrics import parse_answer_output, parse_explicit_output, score_candidate_selection_predictions, score_predictions
...
from .supervision import (
    build_method_supervision, build_oracle_evidence_prompt, build_gold_trace_prompt, build_gold_trace_unmasked_prompt,
    build_graph_free_trace_evaluation_prompt, _explicit_selection_prompt,
)
...
def _causal_lm_loss(model, batch: dict, loss_weights: torch.Tensor) -> torch.Tensor:
    """Compute token-weighted causal-LM loss for joint trace/answer targets."""
    ...
    denominator = (shift_weights * valid).sum().clamp_min(1e-8)
...
def train_rows_once(
    ...
    *,
    ...
    input_tokens = 0
    supervised_tokens = 0
    trace_supervised_tokens = 0
    answer_supervised_tokens = 0
    weighted_supervised_token_sum = 0.0
    padded_tokens = 0
    micro_batches = 0
    optimizer_steps = 0
    ...
    for batch in progress:
        ...
        batch_trace_tokens = int(batch.pop("trace_supervised_token_count"))
        batch_answer_tokens = int(batch.pop("answer_supervised_token_count"))
        batch_weighted_tokens = float(batch.pop("weighted_supervised_token_sum"))
        batch_loss_weights = batch.pop("loss_weights")
        batch.pop("task_ids")
        batch.pop("depths")
        padded_tokens += int(batch["input_ids"].numel())
        with _device_context(device, dtype):
            loss = _causal_lm_loss(model, batch, batch_loss_weights)
        ...
        trace_supervised_tokens += batch_trace_tokens
        answer_supervised_tokens += batch_answer_tokens
        weighted_supervised_token_sum += batch_weighted_tokens
        micro_batches += 1
    ...
    return {
        ...
        "trace_supervised_tokens": trace_supervised_tokens,
        "answer_supervised_tokens": answer_supervised_tokens,
        "weighted_supervised_token_sum": weighted_supervised_token_sum,
...
def _prompt_for_condition(row: dict, condition: str, train_rows: list[dict], config: dict) -> tuple[str, dict | None]:
    if condition == "graph_free":
        if config.get("evaluation", {}).get("graph_free_output_format") == "trace_and_answer":
            return build_graph_free_trace_evaluation_prompt(row["text"]), None
        return build_evaluation_prompt(row["text"]), None
    if condition == "gold_trace":
        return build_gold_trace_prompt(row), {"uses_gold_trace": True, "terminal_masked": True}
    if condition == "gold_trace_unmasked":
        return build_gold_trace_unmasked_prompt(row), {"uses_gold_trace": True, "terminal_masked": False}
    ...
    with torch.inference_mode():
        ...
        for start in progress:
            ...
            for row, prompt, prompt_item, prediction_text in zip(batch_rows, prompts, prompt_info, texts):
                ...
                if condition in {"graph_free", "gold_trace", "gold_trace_unmasked"} and (
                    config.get("evaluation", {}).get("graph_free_output_format") == "trace_and_answer"
                    or condition in {"gold_trace", "gold_trace_unmasked"}
                    or config["protocol"] in {"graph_free_trace", "graph_free_trace_joint", "candidate_index_anti_copy_bridge"}
                ):
                    prediction_answer = parse_answer_output(prediction_text)
                prediction = {
                    ...
                    "uses_gold_trace": bool(info and info.get("uses_gold_trace", False)),
                    "terminal_masked": bool(info.get("terminal_masked", False)) if info else None,
                    "uses_candidate_paths": bool(info and info.get("uses_candidate_paths", False)),
                    "expected_trace": (
                        " -> ".join(
                            [str(node) for node in row["path_nodes"][:-1]]
                            + (["<MASKED_TERMINAL>"] if condition != "gold_trace_unmasked" else [str(row["path_nodes"][-1])])
                        )
                        if condition in {"graph_free", "gold_trace", "gold_trace_unmasked"} and (
                            config.get("evaluation", {}).get("graph_free_output_format") == "trace_and_answer"
                            or condition in {"gold_trace", "gold_trace_unmasked"}
                ...
                if prediction_answer is not None:
                    prediction["prediction_answer"] = prediction_answer
                if condition in {"graph_free", "gold_trace", "gold_trace_unmasked"}:
                    from .metrics import parse_trace_output
                    prediction["prediction_trace"] = parse_trace_output(prediction_text)
                if parsed:
