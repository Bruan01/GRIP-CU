...
def build_candidate_pool_for_query(row: dict, train_rows: list[dict], distractor_count: int, seed: int) -> list[dict]:
    """Build one query pool using the query's gold path plus train-only distractors."""
    if row.get("split") == "train":
        raise ValueError("query pool helper is for validation/test rows; use build_candidate_pools for train")
    if any(candidate.get("split") != "train" for candidate in train_rows):
        raise ValueError("query distractors must come from train split only")
    eligible = [
        candidate for candidate in sorted(train_rows, key=lambda item: item["task_id"])
