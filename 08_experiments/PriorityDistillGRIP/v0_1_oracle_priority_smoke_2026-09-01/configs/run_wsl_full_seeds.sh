...
RUN_ID="${RUN_ID:-wsl3090_priority_distill_full_$(date +%Y%m%d_%H%M%S)}"
cd "$EXPERIMENT_ROOT"
...
"$PYTHON_EXECUTABLE" scripts/run_suite.py --run-id "$RUN_ID" --full-seeds --model-name-or-path "$MODEL_NAME_OR_PATH" --python-executable "$PYTHON_EXECUTABLE" --resume 2>&1 | tee "$log_file"
