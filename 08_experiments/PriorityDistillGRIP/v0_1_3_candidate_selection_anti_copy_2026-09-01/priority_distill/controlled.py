...
from .experiment import evaluate_checkpoint, load_runtime, save_checkpoint, train_rows_once
...
def _prepare_rows(train_rows, tokenizer, config, seed):
    ...
    reference_tokens = sum(int(row["input_token_count"]) for row in all_paths)
    ...
    for epoch in range(1, stage2_epochs + 1):
        if replay_ratio == 0.0:
            rows, budget = balance_rows_to_token_budget(answer, answer_tokens, seed + epoch)
            stage2_segments.append((f"stage2_epoch{epoch}", "answer_only", rows, budget))
            continue
        answer_target = max(1, int(round(answer_tokens * (1.0 - replay_ratio))))
        evidence_target = max(1, answer_tokens - answer_target)
        answer_rows, answer_budget = balance_rows_to_token_budget(answer, answer_target, seed + epoch)
        replay_method = protocol_spec["replay_method"]
        replay_source = {"all_paths_equal_token": all_paths, "all_paths_terminal_masked_equal_token": anti_copy}[replay_method]
        evidence_rows, evidence_budget = balance_rows_to_token_budget(replay_source, evidence_target, seed + epoch + 50000)
        rows = answer_rows + evidence_rows
        budget = {
            "target_input_tokens": answer_tokens,
            "input_tokens": answer_budget["input_tokens"] + evidence_budget["input_tokens"],
            "overshoot_tokens": answer_budget["overshoot_tokens"] + evidence_budget["overshoot_tokens"],
            "examples": len(rows),
            "cycles": max(answer_budget["cycles"], evidence_budget["cycles"]),
            "supervised_tokens": answer_budget["supervised_tokens"] + evidence_budget["supervised_tokens"],
            "truncated_examples": answer_budget["truncated_examples"] + evidence_budget["truncated_examples"],
            "truncated_prompt_tokens": answer_budget["truncated_prompt_tokens"] + evidence_budget["truncated_prompt_tokens"],
            "answer_only_input_tokens": answer_budget["input_tokens"],
            "evidence_replay_input_tokens": evidence_budget["input_tokens"],
            "answer_only_examples": answer_budget["examples"],
            "evidence_replay_examples": evidence_budget["examples"],
        }
        stage2_segments.append((f"stage2_epoch{epoch}", "answer_only_plus_evidence_replay", rows, budget))
    ...
    audit = {
        ...
        "candidate_selection_requires_matching": stage1_method in {"all_paths_equal_token", "all_paths_terminal_masked_equal_token"},
        "terminal_masked": stage1_method == "all_paths_terminal_masked_equal_token",
        "replay_method": protocol_spec["replay_method"],
    }
