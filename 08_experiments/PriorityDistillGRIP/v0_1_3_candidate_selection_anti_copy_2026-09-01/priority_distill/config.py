...
def validate_config(config: dict) -> None:
    ...
    if not str(config.get("experiment_id", "")).endswith("2026-09-01"):
        raise ValueError("v0.1.3 config must be dated 2026-09-01")
    ...
    if config["protocol"] == "candidate_selection_anti_copy_replay" and abs(replay - 0.2) > 1e-9:
        raise ValueError("candidate_selection_anti_copy_replay fixes stage2_replay_ratio at 0.2")
    if config["protocol"] != "candidate_selection_anti_copy_replay" and replay != 0.0:
        raise ValueError("only candidate_selection_anti_copy_replay may use Stage-2 evidence replay")
