...
def generate_predictions(model, tokenizer, rows: list[dict], config: dict, device: torch.device, dtype: torch.dtype, condition: str) -> tuple[list[dict], dict]:
    model.eval()
...
def evaluate_checkpoint(model, tokenizer, splits: dict[str, list[dict]], config: dict, device: torch.device, dtype: torch.dtype, output_dir: Path, checkpoint_name: str) -> dict:
    ...
    for condition in config["diagnostic"]["conditions"]:
        for split in config["diagnostic"]["splits"]:
            prediction_rows, split_metrics = generate_predictions(model, tokenizer, splits[split], config, device, dtype, condition)
