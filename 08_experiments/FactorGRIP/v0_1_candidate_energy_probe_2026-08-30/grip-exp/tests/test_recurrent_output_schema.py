...
class RecurrentOutputSchemaTest(unittest.TestCase):
    ...
    def test_serialized_prediction_contains_mechanism_and_control_fields(self):
        ...
        self.assertEqual(row["recurrent_train_k"], 2)
        self.assertEqual(row["generated_token_count"], 5)
        self.assertTrue(row["ended_with_eos"])
        self.assertTrue(row["response_in_candidates"])
