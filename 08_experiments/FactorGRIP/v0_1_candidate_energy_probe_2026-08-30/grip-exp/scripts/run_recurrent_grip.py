...
def _build_context_samples(
    record: dict, tokenizer, exp_args: dict, graph_id: str
) -> tuple[list[str], dict]:
    title = record.get("title", graph_id)
    formatter = GenGraphContextTask(
    ...
    node_quota = int(exp_args.get("context_node_samples", 0))
    edge_quota = int(exp_args.get("context_edge_samples", 0))
    if node_quota > 0 or edge_quota > 0:
        nodes, edges, manifest = select_stratified_graph_context(
            graph=record["graph"],
            node_quota=node_quota,
            edge_quota=edge_quota,
            seed=int(exp_args.get("context_sampling_seed", 2026)),
            train_samples=record.get("recurrent_questions", []),
        )
        context_samples = []
        for node in nodes:
            fact = formatter.node_context_format.format(node=node, title=title)
            context_samples.append(
                formatter.create_chat_message(formatter.instruction_template.format(title=title), fact)
                if exp_args["format_as_instruction"]
                else fact
            )
        for source, relation, target in edges:
            fact = formatter.edge_context_format.format(
                src=source, rel=relation, tgt=target, title=title
            )
            context_samples.append(
                formatter.create_chat_message(formatter.instruction_template.format(title=title), fact)
                if exp_args["format_as_instruction"]
        ...
        context_samples = formatter.sample_post_process(context_samples)
        manifest.update(
            {
                "graph_id": graph_id,
                "title": title,
                "format_as_instruction": bool(exp_args["format_as_instruction"]),
                "context_upsampling_applied": False,
                "legacy_max_context_samples": 0,
            }
        )
        return context_samples, manifest

    context_samples = formatter()[0]
    max_context_samples = int(exp_args.get("max_context_samples", 0))
    uncapped_count = len(context_samples)
    if max_context_samples:
        context_samples = context_samples[:max_context_samples]
    manifest = {
        "sampling_strategy": (
            "legacy_prefix_cap" if max_context_samples else "full_gen_graph_context"
        ),
        "graph_id": graph_id,
        "title": title,
        "format_as_instruction": bool(exp_args["format_as_instruction"]),
        "context_upsampling_applied": bool(exp_args["context_upsampling"]),
        "legacy_max_context_samples": max_context_samples,
        "uncapped_context_sample_count": uncapped_count,
        "selected_total_count": len(context_samples),
        "audit_warning": (
            "prefix cap may contain only node declarations; use stratified quotas"
            if max_context_samples
            else None
        ),
    }
        ...
        with _adapter_context(model, adapter_control):
            for depth in exp_args["recurrent_depth_sweep"]:
                ...
                for index in tqdm(
                    ...
                    candidate_relations = sample.get("candidate_relations")
                    ended_with_eos = bool(
                        generated_token_ids.numel()
                        and tokenizer.eos_token_id is not None
                        and int(generated_token_ids[-1].item()) == int(tokenizer.eos_token_id)
                    )
                    response_in_candidates = (
                        exact_match(parsed, candidate_relations)
                    ...
                    prediction = RecurrentPrediction(
                        ...
                        true_hop=(
                            int(sample["true_hop"])
                            if sample.get("true_hop") is not None
                            else None
                        ),
                        recurrence_k=int(depth),
                        recurrent_train_k=int(exp_args["recurrent_depth_train"]),
                        ...
                        response_in_candidates=response_in_candidates,
                        step_pooled_hidden_states=trace,
                        metadata={
                            "split": sample["split"],
                            "question_type": sample.get("question_type", "unknown"),
                            "source_node": sample.get("source_node"),
                            "target_node": sample.get("target_node"),
                            "shortest_path": sample.get("shortest_path", []),
                            "structural_distance": sample.get("structural_distance"),
                            "structural_reachable": sample.get("structural_reachable"),
...
def _train_adapters(
    ...
    for graph_index, record in enumerate(records):
        ...
        context_samples, context_manifest = _build_context_samples(
            record=record, tokenizer=tokenizer, exp_args=exp_args, graph_id=graph_id
        )
        adapter_dir = Path(exp_args["adapter_output_dir"]) / safe_graph_id
        adapter_dir.mkdir(parents=True, exist_ok=True)
        (adapter_dir / "context_sampling_manifest.json").write_text(
            json.dumps(context_manifest, ensure_ascii=False, indent=2), encoding="utf-8"
        )
        log_event(
            "recurrent_context_selected",
            graph_id=graph_id,
            sampling_strategy=context_manifest["sampling_strategy"],
            selected_context_samples=len(context_samples),
            selected_node_count=context_manifest.get("selected_node_count"),
            selected_edge_count=context_manifest.get("selected_edge_count"),
            selected_relation_count=context_manifest.get("selected_relation_count"),
            relation_coverage=context_manifest.get("relation_coverage"),
            selection_sha256=context_manifest.get("selection_sha256"),
        )
        ...
        manifest = {
            ...
            "executor_layer_index": executor_index,
            "recurrent_depth_train": exp_args["recurrent_depth_train"],
            "context_sampling_strategy": context_manifest["sampling_strategy"],
            "context_selection_sha256": context_manifest.get("selection_sha256"),
            "context_selected_node_count": context_manifest.get("selected_node_count"),
            "context_selected_edge_count": context_manifest.get("selected_edge_count"),
            "context_selected_relation_count": context_manifest.get("selected_relation_count"),
...
def main() -> None:
    ...
    if exp_args["involve_qa_epochs"] < 1:
        raise ValueError("RecurrentGRIP requires --involve_qa_epochs >= 1 to train the recurrent QA program")
    ...
    if overwrite and output_path.exists():
        output_path.unlink()
    if not records:
        raise ValueError("No recurrent dataset records were loaded")
